...
export default function ShopClientPage({ allProducts, allTags }: { allProducts: Product[], allTags: string[] }) {
  ...
  useEffect(() => {
    ...
    if (priceQuery) {
      const maxPrice = parseInt(priceQuery, 10);
      if (!isNaN(maxPrice)) {
        ...
        <div className="space-y-2">
          {allTags.map(tag => (
            <div key={tag} className="flex items-center space-x-2">
              <Checkbox id={`tag-${tag}`} onCheckedChange={() => handleTagChange(tag)} />
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        <p className="text-sm text-muted-foreground uppercase tracking-widest">Shop</p>
        <h1 className="text-4xl font-headline font-bold text-center">All Products</h1>
      </div>
