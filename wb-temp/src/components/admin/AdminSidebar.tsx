...
import { LayoutDashboard, Package, LogOut, Megaphone, Send, PenSquare, ImageIcon, LayoutGrid, Settings, BookText, TicketPercent, FileText, Wrench, TagIcon, Gift, Camera, Users, ShoppingBag, BarChart2 } from "lucide-react";
...
const navItems = [
  ...
  { href: "/wb-admin/analytics", label: "Analytics", icon: BarChart2 },
  ...
  { href: "/wb-admin/samples", label: "Samples", icon: Camera },
  ...
  { href: "/wb-admin/ads", label: "Ads", icon: Megaphone },
  ...
  const NavContent = () => (
     <div className="flex h-full max-h-screen flex-col gap-2">
        ...
        <div className="flex-1 overflow-y-auto">
            ...
            {navItems.map((item) => {
                const isActive = pathname.startsWith(item.href) && (item.href !== '/wb-admin/dashboard' || pathname === '/wb-admin/dashboard');
                if (item.href === '/wb-admin/leads') {
                    const isLeadsActive = pathname === '/wb-admin/leads';
                    const isEnhancedActive = pathname === '/wb-admin/leads/enhanced-page';
                    return (
                        <div key={item.href}>
                             <Link
                                href={item.href}
                                ...
                                (isLeadsActive || isEnhancedActive) && "bg-muted text-primary"
                                )}
                            >
                             ...
                             <div className="pl-8 space-y-1 mt-1">
                                 <Link href="/wb-admin/leads/enhanced-page" className={cn("block text-xs text-muted-foreground hover:text-primary", isEnhancedActive && "text-primary font-semibold")}>
                                     - Visitor Analytics
                                 </Link>
                             </div>
                        </div>
