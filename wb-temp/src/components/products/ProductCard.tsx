...
const ProductCard = ({ product }: ProductCardProps) => {
  ...
  const categorySlug = slugify(product.category.split(',')[0]);
  const productSlug = slugify(product.name);
  const productUrl = `/collections/${categorySlug}/${productSlug}`
  ...
  const shippingCost = product.price > 2999 ? 0 : 99;
  ...
  return (
    ...
    <Card className="flex flex-col overflow-hidden transition-shadow duration-300 hover:shadow-lg group bg-card h-full relative">
      ...
      <CardContent className="flex-1 p-3 flex flex-col justify-between space-y-2">
        <div>
          <Link href={productUrl}>
            <h3 className="font-headline font-bold text-sm leading-tight group-hover:text-accent transition-colors line-clamp-2">
              {product.name}
            </h3>
          </Link>
        </div>
        <div className="mt-auto">
            ...
            <p className="text-xs font-semibold text-destructive mt-1">Price drops on quantity!</p>
