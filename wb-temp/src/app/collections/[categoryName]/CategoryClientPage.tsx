...
function CategoryClientPage({ allProducts, category }: { allProducts: Product[], category: Category }) {
  ...
  const categoryProducts = useMemo(() => {
    ...
    return allProducts.filter(p => 
      p.category.split(',').map(c => c.trim().toLowerCase().replace(/ /g, '-')).includes(categoryIdLower)
    );
  ...
  const handleTagChange = (checked: boolean | 'indeterminate', tag: string) => {
          ...
          <div className="space-y-2">
            {availableTags.map(tag => (
              <div key={tag} className="flex items-center space-x-2">
                <Checkbox id={`tag-${tag}`} onCheckedChange={(checked) => handleTagChange(checked, tag)} checked={selectedTags.includes(tag)} />
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        ...
        <h1 className="text-4xl font-headline font-bold text-center bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent">
          {category?.name}
        </h1>
      </div>
